...
describe("Failure Analysis — Android", () => {

  it("App launches and home screen is displayed", async () => {
    const root = await browser.$("android=new UiSelector().index(0)");
    await root.waitForExist({ timeout: 20000 });

    const src = await browser.getPageSource();
    if (!src || src.length < 50) throw new Error("App did not load");

    await browser.takeScreenshot();
    await annotate("no_defect");
    await setStatus("passed", "App launched successfully");
  });

  it("Login button is present and tappable", async () => {
    await annotate("automation_bug");
    await setStatus("failed", "Element 'com.example.nonexistent:id/login_btn' not found — locator needs updating");
    throw new Error("Element not found: locator is outdated and needs to be updated in the test script");
  });
