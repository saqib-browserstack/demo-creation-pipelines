...
const { captureAutomateScreenshot } = require('@percy/sdk-utils') as {
  captureAutomateScreenshot: (options: Record<string, unknown>) => Promise<void>;
};
...
const ignoreCartSubtotal = ['[class*="subtotal"], p:has(span[class*="font-bold"])'];
...
async function snap(
    ...
    // excluded from the pixel diff so dynamic content never causes false failures.
    ...
    const withTimeout = <T>(p: Promise<T>, ms: number, label: string): Promise<T> =>
        Promise.race([p, new Promise<T>((_, reject) =>
            setTimeout(() => reject(new Error(`Percy timed out after ${ms}ms — ${label}`)), ms)
        )]);
    ...
    try {
        ...
        console.warn(`[Percy] Snapshot "${name}" skipped: ${(err as Error).message}`);
    }
    ...
    try { await browser.execute(() => true); } catch { /* session may be gone */ }
}
...
describe('FashionStack Percy Visual — Browse, Cart, and Checkout Journeys', () => {
    ...
    it('should capture visual snapshots — browse Men category, add Polo Shirt to cart with Navy color and size L, and complete checkout', async () => {
        ...
        await snap('T001 — Men Category Grid', 'T001: Men → Polo Shirt → Checkout', {
            scope: 'main',
        ...
        await snap('T001 — Polo Shirt Product Page', 'T001: Men → Polo Shirt → Checkout', {
        ...
        await snap('T001 — Polo Shirt Variants Selected', 'T001: Men → Polo Shirt → Checkout', {
        ...
        await browser.execute((el: HTMLElement) => el.click(), viewCartBtn as unknown as HTMLElement);
        await expect(cartPage.heading).toBeExisting();

        await snap('T001 — Cart with Polo Shirt', 'T001: Men → Polo Shirt → Checkout', {
        ...
        await snap('T001 — Order Confirmation', 'T001: Men → Polo Shirt → Checkout', {
    ...
    it('should capture visual snapshots — browse Women category, select first product variants, and add to cart', async () => {
        ...
        await snap('T002 — Women Category Grid', 'T002: Women → First Product → Cart', {
            scope: 'main',
            ignoreSelectors: [...ignoreProductCount],
        });
        ...
        await snap('T002 — Product Detail Page', 'T002: Women → First Product → Cart', {
            ignoreSelectors: [...ignoreHeader],
        });
        ...
        await snap('T002 — Variants Selected', 'T002: Women → First Product → Cart', {
            scope: 'main',
        });
    ...
    it('should capture visual snapshots — register new account, search for dress, add to cart, and complete purchase', async () => {
        ...
        await snap('T003 — Login Modal', 'T003: Register → Search Dress → Checkout', {
            scope: '[role="dialog"], form',
        });

        await browser.execute(() => {
            const btn = Array.from(document.querySelectorAll('button')).find(b => b.textContent?.trim() === 'Sign up here');
            if (btn) (btn as HTMLElement).click();
        });
        await browser.pause(500);

        await snap('T003 — Registration Form', 'T003: Register → Search Dress → Checkout', {
            scope: '[role="dialog"], form',
        });
        ...
        await snap('T003 — Home Page After Registration', 'T003: Register → Search Dress → Checkout', {
            ignoreSelectors: [...ignoreHeader],
        });
        ...
        await snap('T003 — Dress Search Results', 'T003: Register → Search Dress → Checkout', {
            scope: 'main',
            ignoreSelectors: [...ignoreProductCount],
        });
        ...
        await snap('T003 — Dress Variants Selected', 'T003: Register → Search Dress → Checkout', {
            ignoreSelectors: [...ignoreHeader],
        });
        ...
        await snap('T003 — Checkout Page', 'T003: Register → Search Dress → Checkout', {
            ignoreSelectors: [...ignoreHeader],
        });
        ...
        await snap('T003 — Order Confirmation', 'T003: Register → Search Dress → Checkout', {
            ignoreSelectors: [...ignoreHeader, ...ignoreOrderMeta],
        });
    });
    ...
    it('should capture visual snapshots — geo-location special offers, select sale product, and complete checkout', async () => {
        ...
        await snap('T004 — Special Offers Page', 'T004: Offers → Sale Product → Checkout', {
            scope: 'main',
            ignoreSelectors: [...ignoreProductCount],
        });
        ...
        await snap('T004 — Offer Product Detail', 'T004: Offers → Sale Product → Checkout', {
            ignoreSelectors: [...ignoreHeader],
        });
        ...
        await snap('T004 — Cart with Offer Product', 'T004: Offers → Sale Product → Checkout', {
            ignoreSelectors: [...ignoreHeader, ...ignoreCartSubtotal],
        });
        ...
        await snap('T004 — Order Confirmation', 'T004: Offers → Sale Product → Checkout', {
            ignoreSelectors: [...ignoreHeader, ...ignoreOrderMeta],
        });
    });

    // ─── T005 ─────────────────────────────────────────────────────────────────
    // Demonstrates: quick-view component scope snapshot
    it('should capture visual snapshots — quick view eye icon on Women category, select variants, and add to cart', async () => {
        await homePage.open();

        await snap('T005 — Home Page', 'T005: Quick View → Women Product → Cart', {
            ignoreSelectors: [...ignoreHeader],
        });
        ...
        // Scope-based: product grid only before hover
        await snap('T005 — Women Category Grid', 'T005: Quick View → Women Product → Cart', {
            scope: 'main',
            ignoreSelectors: [...ignoreProductCount],
        });
        ...
        await snap('T005 — Product via Quick View', 'T005: Quick View → Women Product → Cart', {
            ignoreSelectors: [...ignoreHeader],
        });
        ...
        await snap('T005 — Variants Selected', 'T005: Quick View → Women Product → Cart', {
            scope: 'main',
        });
        ...
        await snap('T005 — Cart Page', 'T005: Quick View → Women Product → Cart', {
            ignoreSelectors: [...ignoreHeader, ...ignoreCartSubtotal],
        });
    });

    // ─── T006 ─────────────────────────────────────────────────────────────────
    // Demonstrates: broken-image product — pixel diff catches missing image regressions
    it('should capture visual snapshots — Sale category product with broken image, verify product detail, and complete purchase', async () => {
        await homePage.open();

        await snap('T006 — Home Page', 'T006: Sale Broken Image → Checkout', {
            ignoreSelectors: [...ignoreHeader],
        });
        ...
        // Full-page pixel diff — broken images show as empty boxes, caught by Percy
        await snap('T006 — Sale Category Page', 'T006: Sale Broken Image → Checkout', {
            scope: 'main',
            ignoreSelectors: [...ignoreProductCount],
        });
        ...
        // Pixel-by-pixel diff will catch if the broken image placeholder changes
        await snap('T006 — Product with Broken Image', 'T006: Sale Broken Image → Checkout', {
            ignoreSelectors: [...ignoreHeader],
        });
        ...
        await snap('T006 — Variants Selected', 'T006: Sale Broken Image → Checkout', {
            scope: 'main',
        });
        ...
        await snap('T006 — Cart Page', 'T006: Sale Broken Image → Checkout', {
            ignoreSelectors: [...ignoreHeader, ...ignoreCartSubtotal],
        });
        ...
        await snap('T006 — Order Confirmation', 'T006: Sale Broken Image → Checkout', {
            ignoreSelectors: [...ignoreHeader, ...ignoreOrderMeta],
        });
    });
